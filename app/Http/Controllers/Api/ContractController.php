...
class ContractController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $validated = $request->validate([
            ...
            // Neither pricing engine reads this column — each vehicle type's own rule names its
            // method — so it is accepted for the screens that still show it, never demanded.
            'payment_type' => 'nullable|in:per_order,hourly,fixed,hybrid',
            ...
            'driver_payment_method' => [
                ...
                function ($attribute, $value, $fail) use ($request) {
                    foreach ($this->zoneWithoutClientMap(
                        $request->input('driver_pricing_rules'),
                        $request->input('client_pricing_rules'),
                        $value,
                        $request->input('client_payment_method'),
                    ) as $problem) {
                        $fail($problem);
                    }
                },
    ...
    public function show(Contract $contract): JsonResponse
    {
        ...
        return response()->json($contract->load(['client', 'assignments.employee']));
    }
        ...
        $validated = $request->validate([
            ...
            'driver_payment_method' => [
                ...
                function ($attribute, $value, $fail) use ($request, $contract) {
                    ...
                    foreach ($this->zoneWithoutClientMap($driverRules, $clientRules, $value, $clientMethod) as $problem) {
                        $fail($problem);
                    }
                },
            ],
     ...
     *
     * With no per-type rules to read, the two summary columns are the only statement the request
     * makes, so they are what is judged.
     *
     * @param  mixed  $driverRules
     * @param  mixed  $clientRules
     * @return array<int, string>
     */
    private function zoneWithoutClientMap($driverRules, $clientRules, ?string $driverMethod = null, ?string $clientMethod = null): array
    {
        $zoneBased = ['zones', 'zone', 'zones_tiers', 'tiered_zones'];

        if (! is_array($driverRules)) {
            if (in_array($driverMethod, $zoneBased, true) && $clientMethod !== 'zones') {
                return ["طريقة دفع السائق «{$driverMethod}» تعتمد على الفئات، ولا يمكن ذلك ما لم تكن طريقة دفع العميل هي الفئات."];
            }
