...
class Employee extends Model
{
    ...
    protected $fillable = [
        ...
        'pay_type', 'official_salary', 'iban', 'bank_name', 'actual_salary', 'rate_per_order', 'has_end_of_service',
    ...
    /**
     * An IBAN is kept the same way whoever writes it — the form, the importer, a seeder: no
     * spaces, upper case. A blank is no IBAN, not an empty string, so «has no IBAN» is one question.
     */
    public function setIbanAttribute(?string $value): void
    {
        $this->attributes['iban'] = Iban::normalize($value) ?: null;
    }
    ...
    {
        return $this->hasOne(ContractAssignment::class)->current();
    }
