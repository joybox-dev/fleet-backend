...
* **الرابط (URL):** `http://localhost:3000/login` (أو الرابط المعتمد للاستضافة)
* **البريد الإلكتروني:** `mersal@fleetops.kw`
* **كلمة المرور:** `abuhadram` (ملاحظة: يدعم النظام الآن التنظيف التلقائي للمسافات الزائدة `trim()` عند النسخ واللصق بالخطأ).

### 🛠️ التحقق من تحديثات شاشة الدخول (June 1st Hotfixes):
1. **اتجاه اللغة (RTL)**: تأكد من أن الصفحة تظهر بالكامل باللغة العربية والاتجاه الصحيح من اليمين إلى اليسار.
2. **سلاسة الحقول (Symmetric Padding)**: اكتب أحرفاً تجريبية وتأكد من وجود مسافة أمان متناسقة بين النص والرمز الجانبي لحل مشكلة التلاصق نهائياً.
3. **التعبئة التلقائية (WebKit Autofill)**: جرب تسجيل الدخول باستخدام ميزة الحفظ والتعبئة التلقائية بمتصفح Chrome/Edge؛ ستجد أن حقل التعبئة يندمج بنعومة بالغة مع الخلفية الداكنة الفاخرة دون ظهور المربع البرتقالي/الأزرق الفاتح التقليدي.
4. **زر الدخول**: تأكد من اختفاء رابط "Reset Key؟" ودوران سهم الدخول لليسار في اتجاه الـ RTL الطبيعي.
...
graph TD
    ...
    subgraph "3. التدفق الحسابي الأخير"
        ...
        J --> M
        K --> M
        M --> N[صافي راتب البنك: 87.500 د.ك]
...
* **خطوات الفحص (CRUD):**
  1. **الإضافة (Create):** اذهب إلى **الموظفين** $\rightarrow$ **إضافة موظف**:
     ...
     - **تارغت الطلبات شهرياً (Target Orders Monthly):** `41` (في الخطوة الثانية من معالج الإضافة)
     - **سعر الطلب المميز (Premium Commission Rate):** `0.500` د.ك (في الخطوة الثانية من معالج الإضافة)
     - اضغط **حفظ وإنهاء**.
  ...
  3. **مراجعة بطاقة العمولات المحفزة:** ستلاحظ على الفور في ملف أحمد الحربي ظهور كارت تفصيلي يعرض عمولات السائق: التارغت (`41 طلب`)، العمولة الأساسية (`0.250 د.ك`)، والعمولة المميزة (`0.500 د.ك`).
  4. **تعيين السيارة (Vehicle Assignment):** في كارد السيارات بالعمود الجانبي الثابت، اضغط على **تعيين سيارة**:
     - **السيارة:** اختر التويوتا كورولا `99281/4` المتاحة.
     - **تاريخ البدء:** `2026-05-01`
     - **العقد:** اختر عقد Deliveroo المتاح.
     - اضغط حفظ وتأكد من ربط السيارة بالسائق فوراً.
  5. **اختبار فك التعيين بتأريخ رجعي (Unassign with Backdate):** اضغط على علامة `✕ فك التعيين` الملحقة بالسيارة؛ ستفتح لك نافذة منبثقة تفاعلية تطلب تاريخ تسليم السيارة. اختر تاريخاً رجعياً (مثال: أمس) واضغط حفظ وتأكد من انتقال السيارة بنجاح إلى أرشيف السيارات التاريخي للموظف مع بقاء السجل الزمني دقيقاً. أعد تعيينها مجدداً بنفس الخطوات السابقة لمتابعة السيناريو.
...
* **📊 الحصيلة الحسابية الحالية للسجل اليومي:**
  ...
  - إجمالي عمولة السائق المحتسبة: $40 \text{ طلب} \times 0.250 \text{ د.ك} + 5 \text{ طلبات} \times 0.500 \text{ د.ك} = \mathbf{12.500}$ د.ك (عمولة التارغت المتدرجة).
...
* **هدف الفحص:** تسجيل دفعة تسوية كاش، والتحقق من خصمها الفوري وتوزيعها التلقائي بنظام FIFO على السجلات القديمة، وفحص آلية التحديث الذكي والبحث المتقدم في الواجهة.
* **خطوات الفحص (CRUD & UX):**
  1. **التحقق من قائمة الـ 1000 سائق (Show More)**: عند فتح شاشة تسوية الكاش، تأكد من ظهور القائمة الجانبية خفيفة وسريعة وتعرض 25 سائقاً فقط مع وجود زر تفاعلي في أسفل القائمة يحمل عنوان "➕ عرض المزيد" لزيادة العدد تدريجياً.
  2. **اختبار البحث الفوري الذكي وقوة الترتيب اللفظي (Priority Sorting)**:
     - اكتب في صندوق البحث اسم سائق محدد (مثال: `Driver 1`).
     - تأكد من أن السائق `Driver 1` يقفز فوراً ليصبح **البطاقة الأولى في أعلى القائمة** (مستقلاً وبارزاً)، متفوقاً على السائقين المركبة مثل `Driver 11` أو `Driver 12` على الرغم من أن إجمالي المبالغ المعلقة لديهم قد تكون أعلى بكثير!
  3. **الإضافة والتسوية الجزئية (Create & Settle)**: انقر على بطاقة السائق `أحمد الحربي` في القائمة الجانبية، ثم اذهب إلى تفاصيل السند:
     - **المبلغ المستلم:** أدخل `60.000` د.ك (وهو جزء من دينه الكلي `90.000` د.ك)
     - **التاريخ:** تاريخ اليوم.
     - **ملاحظات:** `إرجاع دفعة كاش مايو`
     - انقر على **تأكيد التسوية العامة**.
  4. **فحص آلية التحديث الذكي في نفس المكان (In-Place Update)**:
     - بمجرد الضغط على الحفظ، تأكد من أن **السائق `أحمد الحربي` لا يزال محدداً واللوحة لا تزال مفتوحة أمامه!**
     - تأكد من تحديث أرقام اللوحة الجانبية والبيانات فورياً في نفس اللحظة؛ حيث يعرض النظام الرصيد المتبقي له وهو **`30.000` د.ك**، وإعادة تحميل كشف الأيام الباقية تلقائياً لإبراز السجل اليومي الثاني فقط (بتاريخ 16 مايو كاش معلق `30.000` د.ك) بعد أن أغلق السجل الأول بالكامل.
     - لن تنغلق اللوحة إلا إذا قمت بعمل تسوية إضافية لتصفير دينه بالكامل!
  5. **التحقق المحاسبي الفوري:**
...
* **خطوات الفحص (CRUD):**
  1. **الإضافة (Create):** اذهب إلى **الصيانة والأعطال** (أو من داخل بروفايل الموظف $\rightarrow$ العُهد وسجل السيارات) $\rightarrow$ **تسجيل أمر صيانة**:
     ...
     - اضغط **حفظ**.
  2. **الاعتماد والموافقة (Approval):** بعد حفظ طلب الصيانة، ستجد حالته تظهر كـ `معلقة` (Pending). لكي يتم إدراج خصم الصيانة في مسير رواتب السائق، يجب اعتمادها أولاً:
     - في جدول الصيانة، ابحث عن السجل المضاف وانقر على زر **✅ (اعتماد)** الموجود في عمود الإجراءات (Actions) بجانب السجل.
     - ستفتح نافذة منبثقة تفاعلية لتأكيد التكلفة؛ اترك التكلفة الفعلية `15.000` د.ك واضغط **اعتماد**.
     - ستتغير حالة الطلب فوراً إلى `معتمد` (Approved).
  3. **التحقق:** تأكد من ظهور الصيانة في ملف السائق برصيد خصم معلق بقيمة **`15.000` د.ك**، وأنها أصبحت الآن جاهزة تماماً للاقتطاع في مسير الرواتب.
  4. **إعادة الحساب (Recalculate):** في حال قمت بإنشاء مسير الرواتب مسبقاً، توجه إلى صفحة **الرواتب** واضغط على زر **عرض** ثم زر **⚡ حساب الرواتب** مجدداً ليتم تحديث الأرقام فورياً لتضم خصم الصيانة، مما يعطيك النتائج المطابقة للجدول تماماً!
...
* **خطوات الفحص (CRUD):**
  1. **الإضافة (Create):** اذهب إلى **تقييمات الأداء** (أو من داخل بروفايل الموظف $\rightarrow$ الأداء والانضباط) $\rightarrow$ **إضافة تقييم جديد**:
     ...
     - **الدرجات المدخلة للمعاير الثلاثة المسجلة بالنظام (أدخل الدرجات المقابلة للأوزان):**
       - *أداء العمل (الحد الأقصى 40):* أدخل الدرجة `36` (تُمثل 90%)
       - *الالتزام بالمواعيد (الحد الأقصى 30):* أدخل الدرجة `24` (تُمثل 80%)
       - *خدمة العملاء (الحد الأقصى 30):* أدخل الدرجة `28.5` (تُمثل 95%)
     - اضغط **حفظ**.
  2. **التحقق من عملية الاحتساب الموزون المئوي بالخلفية:**
     $$\text{Evaluation Score} = 36 + 24 + 28.5 = \mathbf{88.50\%}$$
...
| **عمولة الطلبات المنجزة (Orders Bonus)** | `12.500` د.ك | $(40 \text{ طلب} \times 0.250) + (5 \text{ طلبات} \times 0.500)$ (عمولة التارغت المتدرجة) | ✅ متطابق |
| **إجمالي الاستحقاق الداخلي (Gross Actual)** | **`162.500` د.ك** | **المعادلة:** الراتب الداخلي ($150.000$) + عمولة الطلبات ($12.500$) | **🏆 متطابق 100%** |
...
| **صافي الاستحقاق الفعلي النهائي (Net Actual)** | **`87.500` د.ك** | **المعادلة:** إجمالي الاستحقاق ($162.500$) - مجموع الخصومات ($75.000$) | **🏆 متطابق 100%** |
...
* **المعادلة:** $\min(\text{Net Actual}, \text{Official Salary}) \implies \min(87.500, 100.000) = \mathbf{87.500}$ د.ك.
* **النتيجة بالواجهة:** يتم تحويل **`87.500` د.ك** للبنك.
...
* **المعادلة:** $\max(0, \text{Net Actual} - \text{Net Bank}) \implies \max(0, 87.500 - 87.500) = \mathbf{0.000}$ د.ك.
